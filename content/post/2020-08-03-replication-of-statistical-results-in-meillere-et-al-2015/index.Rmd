---
title: Replication of Statistical Results in Meillere et Al (2015)
author: Nathan Brouwer
date: '2020-10-03'
slug: replication-of-statistical-results-in-meillere-et-al-2015
categories: []
tags: []
math: no
meta: no
...
###  Statistical methods - supplemental
...
Note: not specified if log transformation was log base 10 or natural log.  Natural log appears to be used, which is standard.
...
```{r}
telosfull$fledging <- as.factor(telosfull$fledging)
...
```{r}
blmer_cort.trt <- blme::blmer(log(cort) ~ treatment + (1|nest), 
                      data = na.omit(telosfull))
blmer_cort.null <- blme::blmer(log(cort) ~ 1 + (1|nest), 
                                data = na.omit(telosfull))
